...
class DisRefTest(unittest.TestCase):
    ...
    def test_refresh_session_sets_time_initially(self):
        ...
        assert isinstance(parser.parse(reflist[a.pid]), datetime.datetime)

    def test_refresh_session_resets_time(self):
        p = Process()
        a = p.create_reference('foo')
        reflist = json.loads(p.client.get(a.reflist_key) or "{}")
        start = parser.parse(reflist[a.pid])
        a.refresh_session()
        reflist = json.loads(p.client.get(a.reflist_key) or "{}")
        end = parser.parse(reflist[a.pid])
        assert end > start
    ...
    def test_dereference_removes_pid_from_pids(self):
        ...
        assert a.pid in pids
        assert b.pid in pids
        a.dereference()
        pids = json.loads(a.process.client.get(a.reflist_key) or "{}")
        assert a.pid not in pids
        b.dereference()
        pids = json.loads(b.process.client.get(b.reflist_key) or "{}")
        assert b.pid not in pids
        assert len(pids) == 0
...
class UpdateTest(unittest.TestCase):
    ...
    class UserUpdate(Update):
        ...
        def cache(self):
            ...
            client = self.ref.process.client
