...
class Update(object):
    ...
    def __init__(self, process, _id, database='test', collection='test', spec=None, doc=None):
        """
        ...
        self.ref = Reference(process=process, resource=self.resource_id)
    ...
    def __cache(self):
        ...
        if self.ref.get_times_modified() > 0:
            cached = json.loads(self.ref.process.client.get(self.resource_id) or "{}")
            self.merge(cached)
        self.cache() 
