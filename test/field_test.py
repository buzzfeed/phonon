...
class FieldTest(unittest.TestCase):
    ...
    def test_sum_init(self):
        ...
        assert int_sum_field.dtype is int
        assert int_sum_field.operation.__name__ == self.conn.client.incrby.__name__
        assert int_sum_field.default_value == 0

        float_sum_field = phonon.fields.Sum(dtype=float)
        assert float_sum_field.dtype is float
    ...
    def test_sum_cache(self):
        ...
        b = phonon.fields.Sum(dtype=float)
    ...
    def test_diff_init(self):
        ...
        b = phonon.fields.Diff(dtype=float)
        assert b.dtype is float
